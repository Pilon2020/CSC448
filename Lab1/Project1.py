...
with tqdm(total=total_iterations, desc="Computing distance scores") as pbar:
    for i in range(num_sequences):
        for j in range(i, num_sequences):
            ...
            pbar.update(1)
        np.savetxt("nSW_scores.csv", np.array(norm_scores), delimiter=",")


# Save the normalized Smith-Waterman scores as a CSV file

print("Normalized Smith-Waterman scores saved to normalized_smith_waterman_scores.csv")

clustering = AgglomerativeClustering(metric='precomputed',linkage='average',compute_distances=True).fit(dist_scores)
